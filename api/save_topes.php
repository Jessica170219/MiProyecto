...
$stmt = $db->prepare("INSERT INTO topes_mensuales (mes, lavados, gasolina, comida) 
        VALUES (?, ?, ?, ?)
        ON DUPLICATE KEY UPDATE 
            lavados = VALUES(lavados),
            comida = VALUES(comida),
            gasolina = VALUES(gasolina)");

$result = $stmt->execute([$mes, $lavados, $gasolina, $comida]);

if ($result) {
    echo json_encode(['success' => true, 'message' => 'Topes actualizados']);
} else {
