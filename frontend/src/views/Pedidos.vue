...
// ============ FILTRO POR FECHA ============
const tipoFiltroFecha = ref('mes') // 'mes' o 'anio'
const filtroMes = ref(new Date().toISOString().slice(0, 7)) // YYYY-MM
const filtroAnio = ref(new Date().getFullYear()) // YYYY

// Computed que filtra pedidos por fecha y por búsqueda
const pedidosFiltrados = computed(() => {
  if (!Array.isArray(pedidos.value)) return []
  
  // 1. Filtrar por fecha (mes o año)
  let filtrados = [...pedidos.value]
  if (tipoFiltroFecha.value === 'mes' && filtroMes.value) {
    filtrados = filtrados.filter(p => p.fecha.startsWith(filtroMes.value))
  } else if (tipoFiltroFecha.value === 'anio' && filtroAnio.value) {
    filtrados = filtrados.filter(p => p.fecha.startsWith(String(filtroAnio.value)))
  }
  ...
  if (query) {
    filtrados = filtrados.filter(p => 
      ...
      (p.farmacia && p.farmacia.toLowerCase().includes(query)) ||
      p.total.toString().toLowerCase().includes(query)
    )
  }
  return filtrados
})

// Totales de categorías según período (sin búsqueda de texto)
const totalesCategorias = ref({ FARMA: 0, MEDICA: 0, AMBOS: 0, NADIE: 0 })

const fetchTotalesPorPeriodo = async () => {
  try {
    let url = 'http://localhost/MiProyecto/api/get_totales_categorias.php?'
    if (tipoFiltroFecha.value === 'mes') {
      url += `mes=${filtroMes.value}`
    } else {
      url += `anio=${filtroAnio.value}`
    }
    ...
    if (data.success) {
      ...
      console.error('Error al cargar totales del período:', data.message)
    }
  } catch (error) {
    console.error('Error en fetchTotalesPorPeriodo:', error)
  }
}

// Observar cambios en los filtros de fecha y recargar totales
watch([tipoFiltroFecha, filtroMes, filtroAnio], () => {
  fetchTotalesPorPeriodo()
}, { immediate: true })
...
const filtrarProductos = (busqueda) => {
  ...
  return productosDisponibles.value.filter(p => p.nombre.toLowerCase().includes(q))
}
...
// Totales por categoría en el formulario
...
const guardarNuevoPedido = async () => {
  ...
  try {
    ...
    if (result.success) {
      ...
      await fetchPedidos()
      await fetchTotalesPorPeriodo() // actualizar totales filtrados
    } else {
...
<template>
  <div class="min-h-screen bg-[#f0f4f7] font-sans text-slate-700 flex">
    ...
    <header class="flex justify-between items-center mb-6">
      ...
      <button @click="abrirAñadir" class="bg-[#00d084] text-white px-4 py-2 md:px-6 md:py-3 rounded-xl md:rounded-2xl font-bold shadow-lg shadow-green-500/20 hover:scale-105 transition-all text-xs flex items-center gap-2">
        <i class="fas fa-plus"></i> Añadir Pedido
      </button>
    </header>

    <!-- Barra de filtros unificada (fecha + búsqueda) -->
    <div class="bg-white p-3 rounded-2xl shadow-sm mb-6 flex flex-wrap items-center justify-between gap-3 border border-gray-100">
      <!-- Bloque de filtro por fecha -->
      <div class="flex flex-wrap items-center gap-3">
        <div class="flex gap-1">
          <button 
            @click="tipoFiltroFecha = 'mes'" 
            :class="tipoFiltroFecha === 'mes' ? 'bg-[#d65799] text-white' : 'bg-gray-100 text-gray-600'"
            class="px-3 py-1.5 rounded-xl text-xs font-bold transition-all"
          >Mes</button>
          <button 
            @click="tipoFiltroFecha = 'anio'" 
            :class="tipoFiltroFecha === 'anio' ? 'bg-[#d65799] text-white' : 'bg-gray-100 text-gray-600'"
            class="px-3 py-1.5 rounded-xl text-xs font-bold transition-all"
          >Año</button>
        </div>
        <div>
          <input 
            v-if="tipoFiltroFecha === 'mes'"
            type="month" 
            v-model="filtroMes" 
            class="border border-gray-200 rounded-xl px-3 py-1.5 text-sm w-40"
          />
          <input 
            v-else
            type="number" 
            ...
            min="2000"
            max="2100"
            class="border border-gray-200 rounded-xl px-3 py-1.5 text-sm w-28"
          />
        </div>
        <span class="text-xs text-gray-400 hidden sm:inline">| Mostrando {{ pedidosFiltrados.length }} pedidos</span>
      </div>
      
      <!-- Buscador de texto -->
      <div class="flex items-center gap-2 bg-gray-50 px-3 py-1.5 rounded-xl border border-gray-100 flex-1 max-w-sm">
    ...
    <!-- Tarjetas de totales por categoría (más compactas) -->
    <div class="grid grid-cols-2 md:grid-cols-4 gap-3 mb-8">
      <!-- FARMA -->
      <div class="bg-gradient-to-br from-purple-500 to-purple-700 rounded-xl p-3 shadow-lg text-white">
        <p class="text-[10px] font-bold opacity-80">💊 FARMA</p>
        <p class="text-xl font-black mt-0.5">{{ totalesCategorias.FARMA.toFixed(2) }} €</p>
        <p class="text-[9px] opacity-70">{{ tipoFiltroFecha === 'mes' ? filtroMes : `Año ${filtroAnio}` }}</p>
      </div>
      <!-- MÉDICA -->
      <div class="bg-gradient-to-br from-green-500 to-green-700 rounded-xl p-3 shadow-lg text-white">
        <p class="text-[10px] font-bold opacity-80">🏥 MÉDICA</p>
        <p class="text-xl font-black mt-0.5">{{ totalesCategorias.MEDICA.toFixed(2) }} €</p>
        <p class="text-[9px] opacity-70">{{ tipoFiltroFecha === 'mes' ? filtroMes : `Año ${filtroAnio}` }}</p>
      </div>
      <!-- AMBOS -->
      <div class="bg-gradient-to-br from-yellow-500 to-yellow-700 rounded-xl p-3 shadow-lg text-white">
        <p class="text-[10px] font-bold opacity-80">🔄 AMBOS</p>
        <p class="text-xl font-black mt-0.5">{{ totalesCategorias.AMBOS.toFixed(2) }} €</p>
        <p class="text-[9px] opacity-70">{{ tipoFiltroFecha === 'mes' ? filtroMes : `Año ${filtroAnio}` }}</p>
      </div>
      <!-- NADIE -->
      <div class="bg-gradient-to-br from-gray-500 to-gray-700 rounded-xl p-3 shadow-lg text-white">
        <p class="text-[10px] font-bold opacity-80">❌ NADIE</p>
        <p class="text-xl font-black mt-0.5">{{ totalesCategorias.NADIE.toFixed(2) }} €</p>
        <p class="text-[9px] opacity-70">{{ tipoFiltroFecha === 'mes' ? filtroMes : `Año ${filtroAnio}` }}</p>
      </div>
    </div>
            ...
            <tr v-for="pedido in pedidosFiltrados" :key="pedido.id" class="hover:bg-gray-50/50 transition-colors">
              ...
              <td class="p-6 text-right"> 
                <div class="flex gap-2 justify-end">
                  <button @click="verDetallePedido(pedido)" class="bg-blue-100 text-blue-600 p-2 rounded-lg hover:bg-blue-600 hover:text-white transition-all">👁️</button>
                  <button @click="eliminarPedido(pedido.id)" class="bg-red-100 text-red-500 p-2 rounded-lg hover:bg-red-500 hover:text-white transition-all">🗑️</button>
                </div>
              </td>
            </tr>
            <tr v-if="pedidosFiltrados.length === 0">
              <td colspan="4" class="p-12 text-center text-gray-400">No hay pedidos para el período seleccionado</td>
            </tr>
          </tbody>
        </table>
      </div>
      ...
      <div class="md:hidden grid grid-cols-2 gap-3">
        <div v-for="pedido in pedidosFiltrados" :key="pedido.id" class="bg-white p-4 rounded-2xl shadow-sm border border-gray-100 flex flex-col justify-between">
          ...
          <div class="mt-4 pt-3 border-t border-gray-50 flex justify-end gap-2">
            <button @click="verDetallePedido(pedido)" class="w-8 h-8 bg-blue-100 text-blue-600 rounded-lg flex items-center justify-center">👁️</button>
            <button @click="eliminarPedido(pedido.id)" class="w-8 h-8 bg-red-100 text-red-500 rounded-lg flex items-center justify-center">🗑️</button>
          </div>
        </div>
        <div v-if="pedidosFiltrados.length === 0" class="col-span-2 text-center py-8 text-gray-400">
          No hay pedidos para el período seleccionado
        </div>
      </div>
    </main>

    <!-- Botón menú móvil (fuera del main) -->
            ...
            <div>
              ...
              <div class="overflow-x-auto">
                <table class="w-full text-sm border border-gray-100 rounded-xl">
                  <thead class="bg-gray-50 text-gray-400 text-[9px] uppercase">
                    <tr>
                      <th class="p-3 text-left min-w-[200px]">Producto</th>
