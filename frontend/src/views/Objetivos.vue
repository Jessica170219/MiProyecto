...
<template>
  <div class="min-h-screen bg-[#f0f7f4] font-sans text-slate-700">
    <!-- ==================== ASIDE (MENÚ LATERAL) ==================== -->
    <aside 
      :class="[
    ...
    >
      ...
      <header class="flex justify-between items-center mb-6 md:mb-10 fixed md:relative top-0 left-0 w-full p-4 md:p-0 bg-[#f0f7f4] md:bg-transparent z-30">
        <div class="flex items-center gap-3">
          ...
          <div>
            <h1 class="text-xl md:text-2xl font-black text-slate-800">Gestión de Visitas</h1>
            <p class="text-gray-400 font-medium text-xs hidden md:block">Registro y seguimiento comercial</p>
          </div>
        </div>
      ...
      <!-- Tarjeta: Registrar nueva visita -->
      <div class="bg-white p-5 md:p-6 rounded-2xl md:rounded-[2.5rem] shadow-sm border border-green-50 mb-8 relative overflow-hidden mt-14 md:mt-0">
        <div class="absolute -top-10 -right-10 w-40 h-40 bg-[#00d084]/5 rounded-full"></div>
        
        <h3 class="font-black text-slate-800 mb-6 flex items-center gap-2 text-base md:text-lg">
          <span class="w-8 h-8 bg-green-50 text-[#00d084] rounded-xl flex items-center justify-center text-lg">✨</span>
          Registrar nueva visita
        </h3>
        
        <form @submit.prevent="guardarVisita" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 items-end relative z-10">
          <div class="space-y-1">
            <label class="text-[10px] font-black text-gray-400 uppercase ml-2">Fecha</label>
            <input v-model="nuevaVisita.fecha" type="date" class="w-full bg-gray-50 border-none p-3 rounded-xl md:rounded-2xl text-sm font-bold focus:ring-2 focus:ring-[#00d084] outline-none transition-all">
          </div>
          <div class="space-y-1">
            <label class="text-[10px] font-black text-gray-400 uppercase ml-2">Farmacia</label>
            <input v-model="nuevaVisita.farmacia" list="farmaciasList" placeholder="Buscar farmacia..." class="w-full bg-gray-50 border-none p-3 rounded-xl md:rounded-2xl text-sm font-bold focus:ring-2 focus:ring-[#00d084] outline-none transition-all">
          ...
          <div class="space-y-1">
            ...
            <input v-model="nuevaVisita.observacion" type="text" placeholder="¿Qué se trató?" class="w-full bg-gray-50 border-none p-3 rounded-xl md:rounded-2xl text-sm font-bold focus:ring-2 focus:ring-[#00d084] outline-none transition-all">
          </div>
          <button type="submit" class="bg-[#00d084] text-white p-3 rounded-xl md:rounded-2xl font-black shadow-lg shadow-green-500/20 hover:bg-[#00b372] transition-all flex items-center justify-center gap-2">
            ✨ Guardar Visita
          </button>
        </form>
      </div>

      <!-- Tabla / Lista de visitas con filtros -->
      <div class="hidden md:block overflow-x-auto">
        <table class="w-full text-left min-w-[600px]">
          <thead class="text-gray-400 text-[9px] uppercase tracking-[0.3em]">
            <tr>
              <th class="p-4 md:p-8">Calendario</th>
              <th class="p-4 md:p-8">Farmacia / Cliente</th>
              <th class="p-4 md:p-8">Notas de la Visita</th>
              <th class="p-4 md:p-8 text-center">Acción</th>
            </tr>
          </thead>
            ...
            <tr v-for="v in visitas" :key="v.id" class="group hover:bg-green-50/20 transition-colors">
              ...
              <td class="p-4 md:p-8">
               ...
               <td class="p-4 md:p-8 text-center">
                <button @click="eliminarVisita(v.id)" class="text-red-400 hover:text-red-700 transition-colors text-lg">
                  🗑️
                </button>
              </td>
            </tr>
      ...
      <!-- VISTA TARJETA PARA MÓVIL (visible solo en pantallas < 768px) -->
      <div class="md:hidden space-y-4">
        <div v-for="v in visitas" :key="v.id" class="bg-white rounded-2xl shadow-sm border border-gray-100 p-4 relative">
          <!-- Fecha -->
          <div class="flex items-center gap-3 mb-3">
            <div class="w-12 h-12 bg-white border-2 border-green-50 rounded-2xl flex flex-col items-center justify-center shadow-sm">
              <span class="text-[9px] font-black text-green-400 uppercase">{{ new Date(v.fecha).toLocaleString('es-ES', {month: 'short'}) }}</span>
              <span class="text-base font-black text-slate-800">{{ new Date(v.fecha).getDate() }}</span>
            </div>
            <div>
              <p class="text-xs text-gray-400">{{ new Date(v.fecha).getFullYear() }}</p>
              <span class="inline-flex items-center px-2 py-0.5 rounded-full text-[9px] font-black bg-green-100 text-[#00d084]">Completada</span>
            </div>
          </div>
          <!-- Farmacia -->
          <div class="mb-3">
            <p class="font-black text-slate-800 text-base">{{ v.farmacia }}</p>
          </div>
          <!-- Notas -->
          ...
          <!-- Botón eliminar -->
          <button @click="eliminarVisita(v.id)" class="absolute top-3 right-3 text-red-400 hover:text-red-700">
              🗑️
          </button>
        </div>
        <div v-if="visitas.length === 0 && !isLoading" class="text-center py-10 text-gray-400">
